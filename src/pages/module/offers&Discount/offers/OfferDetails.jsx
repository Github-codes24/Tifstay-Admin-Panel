...
export function OfferDetails({ offer, onEdit, onBack }) {
  const navigate = useNavigate();
  
  const handleEdit = () => {
    if (onEdit) {
      // Pass the offer data to the onEdit callback
      onEdit(offer);
    }
  };
    ...
    <div className="flex flex-col gap-6 font-inter">
      ...
      <div className="w-full h-[72px] flex items-center gap-2 bg-white rounded-lg p-4 shadow-sm">
        ...
        <h2 className="text-[24px] font-medium leading-none">
          Offers & Discount / Offers / Offer Details
        </h2>
      </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <div className="h-11 w-full border border-gray-300 rounded-md px-3 py-3 bg-gray-50 text-gray-900">
                    {offer.discountType === 'amount' ? `₹${offer.discount}` : '-'}
                  </div>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-3">
                    <input 
                      type="radio" 
                      checked={!offer.selectedPGHostels || offer.selectedPGHostels.length === 0}
                      readOnly
                      className="w-4 h-4" 
                    />
                ...
                {offer.selectedPGHostels && offer.selectedPGHostels.length > 0 && (
                  <div className="ml-6 mt-4 space-y-3">
                    <label className="text-sm font-medium text-gray-700">Selected PG/Hostel</label>
                    <div className="h-11 w-full border border-gray-300 rounded-md px-3 py-3 bg-gray-50 text-gray-900">
                      {offer.selectedPGHostels.join(', ')}
                    </div>
                  </div>
                )}
              </div>
              ...
              {/* Show In App */}
              <div className="space-y-3">
                <label className="text-sm font-medium text-gray-700">Show This Offer In *</label>
                <div className="space-y-3">
                  <div className="flex items-center space-x-3">
                    <input 
                      type="radio" 
                      checked={offer.showInApp === 'users'}
                      readOnly
                      className="w-4 h-4" 
                    />
                  ...
                  <div className="flex items-center space-x-3">
                    <input 
                      type="radio" 
                      checked={offer.showInApp === 'providers'}
                      readOnly
                      className="w-4 h-4" 
                    />
